...
router.get('/PlatsDeJour', async (req, res) => {
  try {
    // Get desserts
    ...
    // Get all available main dishes (excluding desserts)
    const availableMainDishes = await plats.aggregate([
      { $match: { type: 'plat', _id: { $nin: desserts.map(dessert => dessert._id) } } },
      { $sample: { size: 2 } }
    ]);

    const result = [...desserts, ...availableMainDishes];
